...
export class SliderService {
  ...
  uploadSlider(image: File, alt?: string, order?: number): Observable<SliderImage> {
    const formData = new FormData();
    formData.append('image', image);
    if (alt) formData.append('alt', alt);
    if (order !== undefined) formData.append('order', order.toString());

    return this.http.post<SliderImage>(this.apiUrl, formData, {
  ...
  updateSlider(id: string, alt?: string, order?: number): Observable<SliderImage> {
    const body: any = {};
    if (alt !== undefined) body.alt = alt;
    if (order !== undefined) body.order = order;

    return this.http.put<SliderImage>(`${this.apiUrl}/${id}`, body, {
      headers: this.authService.getAuthHeaders()
    }).pipe(
      map((slider: any) => ({
        ...slider,
  ...
  updateSliderWithImage(id: string, image: File, alt?: string, order?: number): Observable<SliderImage> {
    const formData = new FormData();
    formData.append('image', image);
    if (alt) formData.append('alt', alt);
    if (order !== undefined) formData.append('order', order.toString());

    return this.http.put<SliderImage>(`${this.apiUrl}/${id}/image`, formData, {
      headers: this.authService.getAuthHeaders()
    }).pipe(
      map((slider: any) => ({
        ...slider,
