...
import {
  FeaturedWork,
  FeaturedWorkImage,
} from "../models/featured-work.interface";
...
export class FeaturedWorkService {
  private apiUrl = `${environment.apiUrl}/api/featured-works`;
  ...
  private mapFeaturedWork(work: any): FeaturedWork {
    const mappedUrl =
      work.url && typeof work.url === "string"
        ? work.url.startsWith("http")
          ? work.url
          : `${environment.apiUrl}${work.url}`
        : work.url;
  ...
  uploadFeaturedWork(
    ...
    return this.http
      .post<FeaturedWork>(this.apiUrl, formData, {
  ...
  updateFeaturedWork(
    ...
    return this.http
      .put<FeaturedWork>(`${this.apiUrl}/${id}`, body, {
        headers: this.authService.getAuthHeaders(),
      })
      .pipe(
        map((work: any) => this.mapFeaturedWork(work)),
  ...
  updateFeaturedWorkWithImage(
    ...
    return this.http
      .put<FeaturedWork>(`${this.apiUrl}/${id}/image`, formData, {
        headers: this.authService.getAuthHeaders(),
      })
      .pipe(
        map((work: any) => this.mapFeaturedWork(work)),
  ...
  getInternalImages(featuredWorkId: string): Observable<FeaturedWorkImage[]> {
    return this.http
      .get<FeaturedWorkImage[]>(`${this.apiUrl}/${featuredWorkId}/images`)
      .pipe(
        map((images) => {
  ...
  uploadInternalImage(
    ...
    image: File,
    order?: number,
  ): Observable<FeaturedWorkImage> {
    const formData = new FormData();
    formData.append("image", image);
    if (order !== undefined) formData.append("order", order.toString());

    return this.http
      .post<FeaturedWorkImage>(
        `${this.apiUrl}/${featuredWorkId}/images`,
        formData,
        {
          headers: this.authService.getAuthHeaders(),
        },
  ...
  deleteInternalImage(
    ...
    return this.http
      .delete<void>(`${this.apiUrl}/${featuredWorkId}/images/${imageId}`, {
        headers: this.authService.getAuthHeaders(),
      })
      .pipe(
        catchError((error) => {
